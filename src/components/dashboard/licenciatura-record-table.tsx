...
import { inductionCourses, licenciaturaCursosBasicos, licenciaturaCursosProfesionales } from "@/lib/academic-data";

type LicenciaturaRecordTableProps = {
  records: Course[];
  student: Student;
...
export function LicenciaturaRecordTable({ records, student, onAverageCalculated, thesisGrade, onEditCourse }: LicenciaturaRecordTableProps) {
  const totalRows = 50; // Total rows for Licenciatura
  ...
  useEffect(() => {
    const grades = studentCourses.map(course => {
        const numericGrade = Math.floor(Math.random() * (100 - 70 + 1)) + 70; // Random grade between 70-100
    ...
    <div className="border-2 border-gray-300">
        <Table>
            ...
            <TableBody>
                ...
                {renderCourses(studentCourses.filter(c => licenciaturaCursosBasicos.some(lcb => lcb.name === c.name)), "Cursos Básicos")}
                {renderCourses(studentCourses.filter(c => licenciaturaCursosProfesionales.some(lcp => lcp.name === c.name)), "Cursos Optativos Profesionales")}
