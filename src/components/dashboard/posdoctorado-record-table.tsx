...
type PosdoctoradoRecordTableProps = {
  records: Course[];
  student: Student;
...
export function PosdoctoradoRecordTable({ records, student, onAverageCalculated, thesisGrade, onEditCourse }: PosdoctoradoRecordTableProps) {
  const totalRows = 24; // Empty rows for layout
  ...
  useEffect(() => {
    const grades = studentCourses.map(course => {
        const numericGrade = Math.floor(Math.random() * (100 - 80 + 1)) + 80;
    ...
    const totalCoursesForAverageCalc = studentCourses.length + (thesisGrade ? 1 : 0);

    const average = totalCoursesForAverageCalc > 0 ? Math.round(totalSum / totalCoursesForAverageCalc) : 0;
    onAverageCalculated(average);

  }, [student.studentId, records, thesisGrade, onAverageCalculated, studentCourses.length]);
